...
A> Alternatively, you might consider storing dollars as an `integer` number---of cents!  In U.S. currency there's no real such thing as a fraction of a cent, so money values are actually whole numbers disguised as decimals, from a certain point of view.  By the way, Postgres also offers a `money` type which is essentially a dressed-up integer.  Be careful with that type because it may behave differently depending on your "locale": an American computer would interpret and display it as dollars, while a computer in another country would treat it as that nation's currency, potentially confusing people.
